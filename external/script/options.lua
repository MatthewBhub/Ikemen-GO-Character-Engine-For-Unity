...
options.t_itemname = {
	...
	["portchange"] = function(t, item, cursorPosY, moveTxt)
		if main.f_input(main.t_players, { "$F", "$B", "pal", "s" }) then
			sndPlay(motif.files.snd_data, motif.option_info.cursor_move_snd[1], motif.option_info.cursor_move_snd[2])
			local port = main.f_drawInput(main.f_extractText(motif.option_info.textinput_port_text), txt_textinput, overlay_textinput, motif.option_info.textinput_offset[2], main.f_ySpacing(motif.option_info, "textinput"), motif.optionbgdef)
	...
	["customres"] = function(t, item, cursorPosY, moveTxt)
		if main.f_input(main.t_players, { "pal", "s" }) then
			sndPlay(motif.files.snd_data, motif.option_info.cursor_move_snd[1], motif.option_info.cursor_move_snd[2])
			local width = tonumber(main.f_drawInput(main.f_extractText(motif.option_info.textinput_reswidth_text), txt_textinput, overlay_textinput, motif.option_info.textinput_offset[2], main.f_ySpacing(motif.option_info, "textinput"), motif.optionbgdef))
			if width ~= nil then
				sndPlay(motif.files.snd_data, motif.option_info.cursor_move_snd[1], motif.option_info.cursor_move_snd[2])
				local height = tonumber(main.f_drawInput(main.f_extractText(motif.option_info.textinput_resheight_text), txt_textinput, overlay_textinput, motif.option_info.textinput_offset[2], main.f_ySpacing(motif.option_info, "textinput"), motif.optionbgdef))
				if height ~= nil then
	...
	["customaspect"] = function(t, item, cursorPosY, moveTxt)
		if main.f_input(main.t_players, { "pal", "s" }) then
			sndPlay(motif.files.snd_data, motif.option_info.cursor_move_snd[1], motif.option_info.cursor_move_snd[2])
			local width = tonumber(main.f_drawInput(main.f_extractText(motif.option_info.textinput_aspect_width_text or "Enter aspect width:"), txt_textinput, overlay_textinput, motif.option_info.textinput_offset[2], main.f_ySpacing(motif.option_info, "textinput"), motif.optionbgdef))
			if width ~= nil and width > 0 then
				sndPlay(motif.files.snd_data, motif.option_info.cursor_move_snd[1], motif.option_info.cursor_move_snd[2])
				local height = tonumber(main.f_drawInput(main.f_extractText(motif.option_info.textinput_aspect_height_text or "Enter aspect height:"), txt_textinput, overlay_textinput, motif.option_info.textinput_offset[2], main.f_ySpacing(motif.option_info, "textinput"), motif.optionbgdef))
				if height ~= nil and height > 0 then
					modifyGameOption("Video.FightAspectWidth", width)
					modifyGameOption("Video.FightAspectHeight", height)
					sndPlay(motif.files.snd_data, motif.option_info.cursor_done_snd[1], motif.option_info.cursor_done_snd[2])
					options.modified = true
				else
					sndPlay(motif.files.snd_data, motif.option_info.cancel_snd[1], motif.option_info.cancel_snd[2])
	...
	["keyboard"] = function(t, item, cursorPosY, moveTxt)
		if
			main.f_input(main.t_players, { "pal", "s" }) --[[or getKey():match('^F[0-9]+$')]]
		then
			sndPlay(motif.files.snd_data, motif.option_info.cursor_done_snd[1], motif.option_info.cursor_done_snd[2])
...
function options.f_start()
	...
	for _, v in ipairs(getDirectoryFiles("external/shaders")) do
		v:gsub("^(.-)([^\\/]+)%.([^%.\\/]-)$", function(path, filename, ext)
			...
			if
				ext:match("vert") or ext:match("frag") --[[or ext:match('shader')]]
			then
	...
	if main.debugLog then
		main.f_printTable(options.t_itemname, "debug/t_optionsItemname.txt")
	end
	-- create menu
	...
	-- log
	if main.debugLog then
		main.f_printTable(options.menu, "debug/t_optionsMenu.txt")
	end
end
...
function options.f_keyCfg(cfgType, controller, bgdef, skipClear)
	...
	for i = 1, 2 do
		txt_keyController[i]:update {
			...
			a = motif.option_info["keymenu_item_p" .. i .. "_font"][7],
			height = motif.option_info["keymenu_item_p" .. i .. "_font"][8],
			...
			if i > item - cursorPosY then
				...
				if i == item and j == side then --active item
					...
					t[i].data[j]:update {
						...
						a = motif.option_info.keymenu_item_active_font[7],
						height = motif.option_info.keymenu_item_active_font[8],
					...
					if t[i].vardata ~= nil then
						if t_keyList[joyNum][tostring(t[i]["vardisplay" .. j + player - side])] ~= nil and t_keyList[joyNum][tostring(t[i]["vardisplay" .. j + player - side])] > 1 then
							t[i].vardata[j]:update {
								...
								a = motif.option_info.keymenu_item_value_conflict_font[7],
								height = motif.option_info.keymenu_item_value_conflict_font[8],
							...
							t[i].vardata[j]:update {
								...
								a = motif.option_info.keymenu_item_value_active_font[7],
								height = motif.option_info.keymenu_item_value_active_font[8],
					...
					elseif t[i].infodata ~= nil then
						t[i].infodata[j]:update {
							...
							a = motif.option_info.keymenu_item_info_active_font[7],
							height = motif.option_info.keymenu_item_info_active_font[8],
					...
					t[i].data[j]:update {
						...
						a = motif.option_info.keymenu_item_font[7],
						height = motif.option_info.keymenu_item_font[8],
					...
					if t[i].vardata ~= nil then
						if t_keyList[joyNum][tostring(t[i]["vardisplay" .. j + player - side])] ~= nil and t_keyList[joyNum][tostring(t[i]["vardisplay" .. j + player - side])] > 1 then
							...
							t[i].vardata[j]:update {
								...
								a = motif.option_info.keymenu_item_value_font[7],
								height = motif.option_info.keymenu_item_value_font[8],
					...
					elseif t[i].infodata ~= nil then
						t[i].infodata[j]:update {
							...
							a = motif.option_info.keymenu_item_info_font[7],
							height = motif.option_info.keymenu_item_info_font[8],
	...
	if motif.option_info.menu_boxcursor_visible == 1 then
		local src, dst = main.f_boxcursorAlpha(motif.option_info.menu_boxcursor_alpharange[1], motif.option_info.menu_boxcursor_alpharange[2], motif.option_info.menu_boxcursor_alpharange[3], motif.option_info.menu_boxcursor_alpharange[4], motif.option_info.menu_boxcursor_alpharange[5], motif.option_info.menu_boxcursor_alpharange[6])
		for i = 1, 2 do
			if i == side then
