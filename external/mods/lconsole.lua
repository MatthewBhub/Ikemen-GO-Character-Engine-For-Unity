...
-- Create local proxy functions that call the global Go-registered functions
-- Capture original Go-registered functions so wrappers do not recurse
local _go_spawnChar = _G.spawnChar
local _go_removeChar = _G.removeChar

function spawnChar(playerNumber, defPath, attached)
	-- This calls the original function registered in your script.go file
	lconsole.printString("Spawning character in player slot " .. tostring(playerNumber) .. " with DEF path '" .. tostring(defPath) .. "' and attached = " .. tostring(attached))
	if type(_go_spawnChar) == 'function' then
		return _go_spawnChar(playerNumber, defPath, attached)
	else
		lconsole.printString("Error: underlying spawnChar binding not found")
		return false
	end
end

function removeChar(playerNumber)
	-- This calls the original function registered in your script.go file
	lconsole.printString("Removing character in player slot " .. tostring(playerNumber))
	if type(_go_removeChar) == 'function' then
		return _go_removeChar(playerNumber)
	else
		lconsole.printString("Error: underlying removeChar binding not found")
		return false
	end
end
...
register_man("removeChar", "Clears/destroys the character at the specified player slot.", "playerNumber (integer) - The player slot number to clear.", "true on success")
register_man("spawnChar", "Spawns or reloads a character into the specified player slot.", "playerNumber (integer) - The player slot number to use,\ndefPath (string) - The path to the character .DEF file,\nattached (boolean) (optional) - If true, spawns the character as attached (defaults to false).", "playerID (integer) - The new character's player ID on success, or false on failure.")
